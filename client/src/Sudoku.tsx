...
import { SudokuGameSection } from './components/layout/SudokuSection';
...
export const Sudoku: React.FC<{}> = () => {
  ...
  async function onClickSolve() {
    ...
    const solved = await axios.post('http://localhost:8002/solve',board).catch(e=>{});
    if(solved?.data)
      setGameArray(solved.data.flat().map((x: number) => x.toString()))
      else{
